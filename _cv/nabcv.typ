...
///   Valid keys: "contact", "skills", "values", "hobbies", "references", "publications", "projects".
...
#let cv(
  ...
  // --- Gaps ---
  let gap = (
    sidebar-section-above: 14pt,
    sidebar-section-below: 10pt,
    sidebar-contact-between-items: -6pt,
    sidebar-skill-between-items: 2.5pt,
    sidebar-values-between-items: -5pt,
    sidebar-hobbies-between-items: -5pt,
    sidebar-publication-between-items: 5pt,
    main-section-above: 8pt,
    main-section-below: 8pt,
    main-entry-above: 3pt,
    main-entry-below: 8pt,
    ...
    if show-tl {
      pad(left: gap.section-indent + tl.dot-size / 2 - tl.line-width / 2)[
        #grid(
          ...
          column-gutter: tl.dot-size / 2 - tl.line-width / 2 + tl.dot-to-content,
    ...
    pad(left: gap.section-indent)[
      #block(above: 0pt, below: gap.subsection-entry-below)[
        #pad(left: bullet-offset)[
          ...
          #grid(
            columns: (1fr, auto),
            text(weight: "regular", size: ts.entry-text, title), text(size: ts.entry-date, weight: fw.entry-date, date),
          )
          #v(-5pt)
  ...
  let sidebar-renderers = (
    ...
    contact: () => sidebar-section(si.contact, st.contact)[
      ...
      #let items = ()
      #if phone != none { items.push([#fa-icon("phone") #h(gap.icon-to-text-tight) #phone]) }
      #if email != none {
        items.push([#fa-icon("envelope") #h(gap.icon-to-text-tight) #link("mailto:" + email)[#email]])
      }
      #if profiles != none {
        for profile in profiles {
          let cfg = profiles-config.at(profile.network, default: none)
          if cfg != none {
            items.push([#fa-icon(cfg.icon) #h(gap.icon-to-text-tight) #link(
      ...
      #if address != none {
        let addr-content = if type(address) == array { address.join(linebreak()) } else { address }
        items.push([
          #box(grid(
            columns: (auto, 1fr),
            column-gutter: gap.icon-to-text,
            fa-icon(address-icon), addr-content,
          ))
        ])
      }
      #stack(spacing: 5pt, ..items)
    ],
    skills: () => {
      if skills != none {
    ...
    projects: () => {
      if projects != none {
        sidebar-section(si.projects, st.projects)[
          #show: section-text("projects")
          #v(3pt)
          #for proj in projects [
            #text(weight: fw.skill-group)[#parse-markup(proj.title)] \
            #v(0pt)
            #parse-markup(proj.description) \
            #v(gap.sidebar-skill-between-items)
          ]
        ]
      }
    ...
    summary: () => {
      if summary != none {
        main-section(si.summary, st.summary)[
          #text-block("summary", parse-markup(summary))
        ]
      }
    },
    motivation: () => {
      ...
      [
        ...
        #pad(left: gap.section-indent, right: 0pt)[
          #text(
            ...
            #if headline != none { parse-markup(headline) }
          ]
        ]
